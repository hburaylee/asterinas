...
impl PvclockPage {
    ...
    /// Returns the virtual address of the pvclock page.
    fn addr(&self) -> *const PvclockVcpuTimeInfo {
    ...
    fn read_version(&self) -> u32 {
        // SAFETY: `self.addr()` points to a live KVM pvclock page.
        unsafe { core::ptr::addr_of!((*self.addr()).version).read_volatile() }
    }

    fn read_tsc_to_system_mul(&self) -> u32 {
        // SAFETY: `self.addr()` points to a live KVM pvclock page.
        unsafe { core::ptr::addr_of!((*self.addr()).tsc_to_system_mul).read_volatile() }
    }

    fn read_tsc_shift(&self) -> i8 {
        // SAFETY: `self.addr()` points to a live KVM pvclock page.
        unsafe { core::ptr::addr_of!((*self.addr()).tsc_shift).read_volatile() }
    }
}
