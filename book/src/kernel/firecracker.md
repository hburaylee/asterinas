...
`BOOT_PROTOCOL=pvh` automatically uses the `direct-elf` boot method
...
Note: a PVH build is PVH-only.
The Multiboot and Multiboot2 entry points are disabled
when the `pvh_boot` feature is enabled.
